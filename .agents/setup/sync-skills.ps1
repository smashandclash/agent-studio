...
$TopLevelSkills = Join-Path $Root "skills"
...
New-Item -ItemType Directory -Force -Path $TopLevelSkills | Out-Null
...
function Generate-TopLevel {
    Get-ChildItem -Path $TopLevelSkills -Directory -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    Get-ChildItem -Path $AgentSkills -Directory | ForEach-Object {
        ...
        $DstDir = Join-Path $TopLevelSkills $SkillName
        $Dst = Join-Path $DstDir "SKILL.md"
        New-Item -ItemType Directory -Force -Path $DstDir | Out-Null
        Copy-Item -Path $Src -Destination $Dst -Force
    }
    Write-Output "Generated top-level skills/ from canonical .agents"
}

switch ($Mode) {
    "bootstrap-agents" { Bootstrap-Agents }
    "generate-claude" { Generate-Claude }
    "generate-toplevel" { Generate-TopLevel }
    "full" {
        Bootstrap-Agents
        Generate-Claude
        Generate-TopLevel
    ...
    default {
        throw "Usage: sync-skills.ps1 [bootstrap-agents|generate-claude|generate-toplevel|full]"
    }
}
