...
| Factor | Godot 4 | Unity | Unreal Engine 5 | Web (Three.js + TypeScript) |
|--------|---------|-------|-----------------|-----------------------------|
| **Best for** | 2D games, small 3D, solo/small teams | Mobile, mid-scope 3D, cross-platform | AAA 3D, photorealism, large teams | Instant-play browser games, lightweight multiplayer, rapid iteration |
| **Language** | GDScript (+ C#, C++ via extensions) | C# | C++ / Blueprint | TypeScript / JavaScript |
| **Cost** | Free, MIT license | Free under revenue threshold | Free under revenue threshold, 5% royalty | Free ecosystem, hosting costs vary |
| **Learning curve** | Gentle | Moderate | Steep | Moderate |
| **2D support** | Excellent (native) | Good (but 3D-first engine) | Possible but not ideal | Strong (canvas/WebGL libraries) |
| **3D quality ceiling** | Good (improving rapidly) | Very good | Best-in-class | Good to very good (web constraints apply) |
| **Web export** | Yes (native) | Yes (limited) | No | Native platform |
| **Console export** | Via third-party | Yes (with license) | Yes | Not direct (requires ports/wrappers) |
| **Open source** | Yes | No | Source available | Yes (core libraries) |
...
**For Web (Three.js):**
```markdown
- **Engine**: Web (Three.js) [version]
- **Language**: TypeScript
- **Build System**: Node.js toolchain (Vite/Webpack/esbuild)
- **Asset Pipeline**: Browser-optimized web assets (glTF, compressed textures, lazy loading)
```
...
**For Web (TypeScript):**
- Classes/Types: PascalCase (e.g., `PlayerController`, `CombatState`)
- Variables/functions: camelCase (e.g., `moveSpeed`, `updateLoop`)
- Constants: UPPER_SNAKE_CASE (e.g., `MAX_FRAME_DELTA_MS`)
- Files: kebab-case or domain-oriented snake_case (team-consistent)
- Prefer explicit interfaces and discriminated unions for game state
...
- Three.js/Web stack: training data likely covers up to ~r165-era patterns
