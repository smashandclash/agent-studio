...
This is a complete agent architecture for game development. It
...
for Godot, Unity, Unreal, and Web game stacks — each with dedicated sub-specialists for
...
Cross-host compatibility is handled through `.agents/shared/` with thin adapters
in `.cursor/`.
...
| Get web game architecture advice | `web-specialist` |
| Design Three.js rendering | `threejs-specialist` |
| Build TypeScript web game systems | `typescript-webgame-specialist` |
...
| `/team-web` | Orchestrate web game features end-to-end (web-specialist + Three.js + TypeScript) |
| `/investigate` | Systematic root-cause debugging before implementing fixes |
| `/qa-web` | Browser-centered QA pass with reproducible bug reports |
| `/ship-release` | Release readiness and go/no-go execution workflow |
| `/document-release` | Sync docs to shipped implementation changes |
| `/retro-weekly` | Weekly engineering retrospective workflow |
| `/careful` | Activate high-caution mode for risky operations |
| `/freeze` | Restrict edits to a specified path boundary |
| `/guard` | Enable both careful and freeze safety semantics |
| `/unfreeze` | Remove active freeze/guard boundary restrictions |
