...
Canonical skills live in `.claude/skills/`. For host-neutral compatibility and
optional public extensions, see `.agents/README.md` and `.agents/skills/skills.sh-curated.md`.
...
| `/team-web` | Orchestrate web game delivery across architecture, rendering, TypeScript runtime, and QA |
| `/investigate` | Root-cause debugging workflow with evidence-first investigation |
| `/qa-web` | Browser-focused QA pass with reproducible defect reporting and regression checklist |
| `/ship-release` | Release execution workflow with go/no-go readiness report |
| `/document-release` | Sync project docs to shipped behavior and changes |
| `/retro-weekly` | Weekly retrospective for delivery, quality, blockers, and next actions |
| `/careful` | Enable high-caution mode for risky operations |
| `/freeze` | Restrict edits to an approved path boundary |
| `/guard` | Enable careful + freeze together (maximum safety mode) |
| `/unfreeze` | Remove freeze/guard boundary restrictions |
